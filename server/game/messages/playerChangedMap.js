...
const { WOLF_COMPANION_HP, TIGER_COMPANION_HP, MAP_TOWN } = require("../constants");
...
function handlePlayerChangedMap(room, state, client, data) {
    ...
    // Tiger companion — if wolf is already active and no tiger yet, grant tiger on next summon
    if (INTERIOR_MAPS.has(data.map) && p.wolfHp > 0 && p.tigerHp === 0 && !p.tigerSummonCountdown) {
        const ownedTypes = new Set(
            state.beasts.filter(b => b.tamedBy === client.sessionId).map(b => b.key)
        );
        if (ownedTypes.has("wolf") && ownedTypes.has("tiger") && ownedTypes.has("spider")) {
            p.tigerSummonCountdown = WOLF_SUMMON_DELAY;
            setTimeout(() => {
                if (state.players[client.sessionId] && p.tigerSummonCountdown > 0) {
                    client.send("BEAST_SUMMONING", { duration: WOLF_SUMMON_DELAY });
                }
            }, 300);
        }
